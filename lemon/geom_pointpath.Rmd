...
Connected points
Description
geom_pointpath combines geom_point and geom_path, such that a) when jittering is used, both
lines and points stay connected, and b) provides a visual effect by adding a small gap between the
point and the end of line. geom_pointline combines geom_point and geom_path.
...
distance
* Gap size between point and end of lines; use unit. Is converted to ’pt’ if given as
simple numeric. When NULL or NA, gapping and shorten/treshold is disabled.
To keep the latter, set to 0.
shorten, threshold
When points are closer than threshold, shorten the line by the proportion in
shorten instead of adding a gap by distance.
* lineend Line end style (round, butt, square).
* linejoin Line join style (round, mintre, bevel).
* linemitre Line mitre limit (number greater than 1).12
geom_pointpath
* linesize
Width of of line.
...
p + geom_pointline(aes(colour = factor(cyl)))geom_siderange
13
...
p
Change scales
+ geom_pointline(aes(colour = cyl)) + scale_colour_gradient(low = "blue")
+ geom_pointline(aes(colour = cyl), linecolour='black') + scale_colour_gradient(low = "blue")
+ geom_pointline(aes(shape = factor(cyl))) + scale_shape(solid = FALSE)
...
However, if linecolour is not waiver(), connections will be made between groups, but possible
