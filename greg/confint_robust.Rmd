...
The confint.lm uses the t-distribution as the default confidence interval estimator. When there is
reason to believe that the normal distribution is violated an alternative approach using the vcovHC()
may be more suitable.
\n#### Usage\n<pre><code>\n
confint_robust(object, parm, level = 0.95, HC_type = "HC3",
t_distribution = FALSE, ...)
forestplotCombineRegrObj 7
Arguments
...
t_distribution A boolean for if the t-distribution should be used or not. Defaults to FALSE.
According to Cribari-Nieto and Lima’s study from 2009 this should not be the
case.
