...
rather a cell typically containing a numerical value).26
PivotCell
...
workingData The data filters and batchNames used applied when running calculations (including
the filters needed for base calculations when calculation type="calculation").
evaluationFilters The final and actual data filters used in the calculation of the cell value (e.g.
custom calculation functions can override the working filters).
...
baseStyleName The name of the style applied to this cell (a character value). The style must exist
in the PivotStyles object associated with the PivotTable.
